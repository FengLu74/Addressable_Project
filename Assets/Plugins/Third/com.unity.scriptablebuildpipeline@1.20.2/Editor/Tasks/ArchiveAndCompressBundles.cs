...
#pragma warning restore 649
        ...
        internal static void CopyFileWithTimestampIfDifferent(string srcPath, string destPath, IBuildLogger log, ulong offset = 0)
        {
            if (srcPath == destPath)
                return;
            ...
            using (log.ScopedStep(LogLevel.Verbose, "Copying From Cache", $"{srcPath} -> {destPath}"))
            {
                ...
                #region AES Encrypt
                AESUtils.EncryptFileOffset(destPath, offset);
                #endregion
            }
        }
        ...
        static internal ReturnCode Run(TaskInput input, out TaskOutput output)
        {
            ...
            using (input.Log.ScopedStep(LogLevel.Info, "CopyingCachedFiles"))
            {
                foreach (ArchiveWorkItem item in cachedItems)
                {
                    ...
                    item.ResultDetails.Offset = (ulong)Encoding.UTF8.GetBytes(item.ResultDetails.FileName.GetHashCode().ToString()).Length;
                    CopyFileWithTimestampIfDifferent(item.CachedArtifactPath, item.ResultDetails.FileName, input.Log, item.ResultDetails.Offset);
                }
            }
        ...
        static private void ArchiveSingleItem(ArchiveWorkItem item, string tempOutputFolder, IBuildLogger log)
        {
            using (log.ScopedStep(LogLevel.Info, "ArchiveSingleItem", item.BundleName))
            {
                ...
                item.ResultDetails.Offset = (ulong)Encoding.UTF8.GetBytes(item.ResultDetails.FileName.GetHashCode().ToString()).Length;
                CopyFileWithTimestampIfDifferent(writePath, item.ResultDetails.FileName, log, item.ResultDetails.Offset);
            }
        }
