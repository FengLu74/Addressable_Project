        ...
        private void BeginOperation()
        {
            ...
            if (loadType == LoadType.Local)
            {
...
#endif
                {
                    #region AES Encrypt
                    try {
                        //Log.LogInfo($"m_TransformedInternalId:{m_TransformedInternalId}");
                        // var fileData = AESUtils.DecryptStreamingFileByFullPath(m_TransformedInternalId);
                        // m_RequestOperation = AssetBundle.LoadFromMemoryAsync(fileData, m_Options == null ? 0 : m_Options.Crc);
                        m_RequestOperation = AssetBundle.LoadFromFileAsync(m_TransformedInternalId, m_Options == null ? 0 : m_Options.Crc, m_Options == null ? 0 : m_Options.Offset);
                    } catch (Exception e) {
                        Debug.LogError(e);
                    }
